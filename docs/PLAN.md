...
- **做法**：
  ...
  - 基礎間隔 150ms，每吃 5 顆縮短 10ms，最快到 60ms（Lv.10 封頂）
  - 分數顯示等級 `Lv.X` 及進度條
...
- **做法**：
  ...
  - `handleKeydown` 中 `e.key === 'r' || e.key === 'R'` 時呼叫 `togglePause()`
  - 提示文字改為「按任意鍵開始，按 R 鍵暫停」

#### 重新開始
- **調整**：結束畫面重新開始改為特定按鍵
- **做法**：
  - `handleKeydown` 在 gameover 狀態時，只有方向鍵或空白鍵才觸發儲存並重新開始
  - 先呼叫 `saveScore()` 再呼叫 `startGame()`
  - 提示文字改為「按空白鍵或方向鍵重新開始」
...
- **需求**：按空白鍵在玩家蛇右側生成一條守護蛇
- **做法**：
  - 按空白鍵觸發 `activateDefender()`
  - 守護蛇在玩家蛇右側生成，長度 1（淡青綠色 `#a3e4d7`，頭部 `#48c9b0`）
  - 100% 追蹤 AI 蛇，速度為玩家蛇的 1.5 倍
  - 持續 3 秒後消失
  - 撞到 AI 蛇時 AI 蛇眩暈 2 秒（`aiStunned` 旗標）
  - 技能冷卻 5 秒（`defenderCooldown`）
  - 冷卻時右側顯示技能冷卻狀態
...
- **做法**：
  ...
  - 各自獨立食物、方向、碰撞判斷
  - 鏡像操控模式：左蛇操控反向僅左右相反，上下同向
  - 食物必須兩邊都吃完才重生
  - 網格間距移除 `gap: 0`

#### 改回單蛇模式
- **需求**：移除雙蛇模式，改回原本的單人蛇遊戲
- **做法**：
  - 網格改回 20×20
  - 移除雙蛇相關變數
  - 簡化 `getCellType()` 只處理單一蛇
  - 移除鏡像操控邏輯
...
#### 結束畫面輸入名字問題
- **問題**：在結束畫面輸入名字時，按鍵會觸發儲存並重新開始
- **解決**：`handleKeydown` 在 gameover 狀態下，只有方向鍵或空白鍵才會儲存分數並重新開始
